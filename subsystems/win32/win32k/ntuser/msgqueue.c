...
ClearMsgBitsMask(PUSER_MESSAGE_QUEUE Queue, UINT MessageBits)
{
   ...
   if (MessageBits & QS_MOUSEMOVE) // Odyssey hard coded.
   ...
   if (MessageBits & QS_TIMER) // Odyssey hard coded.
   ...
   if (MessageBits & QS_PAINT) // Odyssey hard coded.
...
MsqPostHotKeyMessage(PVOID Thread, HWND hWnd, WPARAM wParam, LPARAM lParam)
{
   ...
   Mesg.hwnd    = hWnd;
   Mesg.message = id != IDHOT_ODYSSEY ? WM_HOTKEY : WM_SYSCOMMAND;
   Mesg.wParam  = id != IDHOT_ODYSSEY ? wParam    : SC_HOTKEY;
   Mesg.lParam  = id != IDHOT_ODYSSEY ? lParam    : (LPARAM)hWnd;
   Type         = id != IDHOT_ODYSSEY ? QS_HOTKEY : QS_POSTMESSAGE;
   KeQueryTickCount(&LargeTickCount);
