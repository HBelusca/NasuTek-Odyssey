          ...
          switch (lpnm->code)
            {
              ...
              case PSN_WIZNEXT:
                ...
                {
                  if (0 == LoadStringW(hDllInstance, IDS_ODYSSEY_SETUP, Title, sizeof(Title) / sizeof(Title[0])))
                  {
                    wcscpy(Title, L"Odyssey Setup");
                  }
                ...
                if (SetupData->UnattendSetup)
                  {
                    ...
                    if (GetRosInstallCD(szPath, MAX_PATH))
                      {
                        swprintf(szBuffer, L"rundll32.exe shell32.dll,Control_RunDLL intl.cpl,,/f:\"%S\\odyssey\\unattend.inf\"", szPath);
                      }
                    else
                      {
  ...
  switch (uMsg)
    {
	  ...
	  if (oldActivityID != RegistrationNotify->ActivityID)
	    ...
	    oldActivityID = RegistrationNotify->ActivityID;
          ...
          if (NULL != RegistrationNotify->ErrorMessage)
            {
              if (0 == LoadStringW(hDllInstance, IDS_ODYSSEY_SETUP,
 ...
 * BUG 1
 * If there are more than one CDDrive in it containing a Odyssey
...
GetRosInstallCD(WCHAR * szPath, DWORD dwPathLength)
{
  ...
  for (dwIndex = 0; dwIndex < dwLength; dwIndex++)
  {
    ...
    if (GetDriveTypeW(szDrive) == DRIVE_CDROM)
    {
        ...
        wcscat(szBuffer, L"odyssey\\system32\\ntoskrnl.exe");
...
ProcessUnattendSetup()
{
  ...
  wcscat(szPath, L"odyssey\\unattend.inf");
