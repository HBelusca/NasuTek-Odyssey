	...
	do {
		if (current_line != start_line)
		{
			if (!current_line || current_line->index + delta > current_position - es->text)
			{
                ...
                if (new_line == NULL) // odyssey r33509
                    break; // odyssey r33509
...
static void EDIT_LockBuffer(EDITSTATE *es)
{
	if (!es->text) {
	    ...
	    CHAR *textA = NULL; // Odyssey Hacked!
	    UINT countA = 0;
...
static BOOL EDIT_EM_SetTabStops(EDITSTATE *es, INT count, const INT *tabs)
{
	...
	if (!count)
        ...
        if (es->tabs == NULL) // odyssey r33503
        {
            es->tabs_count = 0;
            return FALSE;
        } // odyssey r33503
...
static BOOL EDIT_EM_Undo(EDITSTATE *es)
{
    ...
    if (utext == NULL) // odyssey r33503
        return FALSE; // odyssey r33503
...
//		es = NULL; odyssey
	...
	/* odyssey: check GetWindowLong in case es has been destroyed during processing */
