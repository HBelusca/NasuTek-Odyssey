...
Language=English
{GUID Substitution}
During the translation of a global identifier (GUID) to a Odyssey security ID (SID), no administratively-defined GUID prefix was found.
...
{EXCEPTION}
Cannot Continue
Odyssey cannot continue from this exception.
...
Language=English
{Bad Image}
%hs is either not designed for Odyssey or it contains an error.
...
Language=English
The SAM database on a Odyssey Server is significantly out of synchronization with the copy on the Domain Controller. A complete synchronization is required.
...
Language=English
The NtCreateFile API failed. This error should never be returned to an application, it is a place holder for the Odyssey Lan Manager Redirector to use in its internal error mapping routines.
...
Odyssey was unable to save all the data for the file %hs. The data has been lost.
...
Language=English
{Odyssey Evaluation Notification}
Your Odyssey will NEVER expire!.
...
The relocation occurred because the DLL %hs occupied an address range reserved for Odyssey system DLLs. The vendor supplying the DLL should be contacted for a new DLL.
...
Language=English
The Odyssey I/O reparse tag passed for the NTFS reparse point is invalid.
...
Language=English
The Odyssey I/O reparse tag does not match the one present in the NTFS reparse point.
...
Your system is low on virtual memory. Odyssey is increasing the size of your virtual memory paging file.
...
Language=English
Odyssey doesn't have any Small Business Support editions, so this error never appears.
...
Language=English
This version of Odyssey is not compatible with the behavior version of directory forest, domain or domain controller.
...
Language=English
The error specified is not a valid Odyssey RPC error code.
...
Language=English
Odyssey was not able to process the application binding information.
