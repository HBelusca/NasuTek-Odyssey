 ...
 *                  Hervé Poussineau (hpoussin@odyssey.org)
 */
...
CheckUnattendedSetup(VOID)
{
    ...
    if (_wcsicmp(Value, L"$Odyssey$") != 0)
    {
        DPRINT("Signature not $Odyssey$\n");
        SetupCloseInfFile(UnattendInf);
        return;
    }
                       ...
                       PINPUT_RECORD Ir)
{
    ...
    /*
    * FIXME:
    * Install directories like '\odyssey\test' are not handled yet.
    */
...
RunUSetup(VOID)
{
    ...
    while (Page != REBOOT_PAGE)
    {
        ...
        //CONSOLE_SetUnderlinedTextXY(4, 3, " Odyssey " KERNEL_VERSION_STR " Setup ");
        //CONSOLE_Flush();
