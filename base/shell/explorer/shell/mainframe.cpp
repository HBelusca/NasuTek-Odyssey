...
bool MainFrameBase::ProcessMessage(UINT nmsg, WPARAM wparam, LPARAM lparam, LRESULT* pres)
{
	switch(nmsg) {
	  ...
	  case WM_SIZE: {
#ifdef __ODYSSEY__	///@todo Work around to display rebar in ROS (with flickering) as long as the control isn't fixed
		int height = SendMessage(_hwndrebar, RB_GETBARHEIGHT, 0, 0);
		MoveWindow(_hwndrebar, 0, 0, LOWORD(lparam), height, TRUE);
#else
...
#ifndef __ODYSSEY__ // don't insert reg button for ROS. Regedit should be used.
...
#else
		create_webchildwindow(WebChildWndInfo(_hmdiclient, TEXT("http://www.odyssey.org")));
#endif
		break;
