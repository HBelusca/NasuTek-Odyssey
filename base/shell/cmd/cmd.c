 ...
 *        Workaround because argc[0] is NULL under Odyssey
 *
...
GetEnhancedVar(TCHAR **pFormat, LPTSTR (*GetVar)(TCHAR, BOOL *))
{
	...
	*pFormat = FormatEnd + 1;
	 ...
	 * C:\ReAcToS -> C:\Odyssey) and, if requested with the S modifier,
...
Initialize()
{
	...
	/* Some people like to run Odyssey cmd.exe on Win98, it helps in the
	...
	if (!*ptr)
	{
		...
		ConOutResPrintf(STRING_ODYSSEY_VERSION, 
			_T(KERNEL_RELEASE_STR),
			_T(KERNEL_VERSION_BUILD_STR));
		ConOutPuts(_T("(C) Copyright 1998-") _T(COPYRIGHT_YEAR) _T(" Odyssey Team."));
	}
