/*
 * buildno - Generate the build number for Odyssey
 *
...
write_h (int build, char *buildstr, long revno)
{
	...
	s = s + sprintf (s, "#ifndef _INC_ODYSSEY_BUILDNO\n" );
	s = s + sprintf (s, "#define _INC_ODYSSEY_BUILDNO\n" );
	...
	s = s + sprintf (s, "%s\"\n", build_tag);
	s = s + sprintf (s, "#define ODYSSEY_DLL_VERSION_MAJOR\t%d\n", dllversion);
	s = s + sprintf (s, "#define ODYSSEY_DLL_RELEASE_RC\t\"%d.%d",
	...
	s = s + sprintf (s, "%s\\0\"\n", build_tag);
	s = s + sprintf (s, "#define ODYSSEY_DLL_RELEASE_STR\t\"%d.%d",
	                 dllversion,
	                 KERNEL_VERSION_MINOR);
	if (0 != KERNEL_VERSION_PATCH_LEVEL)
	{
	...
	s = s + sprintf (s, "%s\"\n", build_tag);
	s = s + sprintf (s, "#define ODYSSEY_DLL_VERSION_RC\t\"%d.%d",
	                 dllversion,
	                 KERNEL_VERSION_MINOR);
	if (0 != KERNEL_VERSION_PATCH_LEVEL)
	{
	...
	s = s + sprintf (s, "%s\\0\"\n", build_tag);
	s = s + sprintf (s, "#define ODYSSEY_DLL_VERSION_STR\t\"%d.%d",
	                 dllversion,
	                 KERNEL_VERSION_MINOR);
	if (0 != KERNEL_VERSION_PATCH_LEVEL)
	{
...
main (int argc, char * argv [])
{
	...
	if (! quiet)
	{
		printf ( "\nOdyssey Build Number Generator\n\n");
	}
