...
FrLdrStartup(ULONG Magic)
{
    ...
    while(OldModCount != LoaderBlock.ModsCount)
    {
        ...
        for(i = 0; i < LoaderBlock.ModsCount; i++)
        {
            ...
            {
                ModulesTreated[i] = 1;
                ModHeader = ((PCHAR)odyssey_modules[i].ModStart);
                if(ModHeader[0] == 'M' && ModHeader[1] == 'Z')
                    LdrPEFixupImports
                        ((PVOID)odyssey_modules[i].ModStart,
                         (PCHAR)odyssey_modules[i].String);
            }
        }        
    }
 ...
 * INTERNAL
 *
 *     Sets up the Odyssey Startup Page Directory.
 *
 * Params:
 *     None.
 *
...
FrLdrMapModule(FILE *KernelImage, PCHAR ImageName, PCHAR MemLoadAddr, ULONG KernelAddr)
{
    ...
    ModuleData = &odyssey_modules[LoaderBlock.ModsCount];
                ...
                PULONG ModuleSize)
{
    ...
    NameBuffer = odyssey_module_strings[LoaderBlock.ModsCount];
